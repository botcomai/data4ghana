...
// ─── Cleanheart Solutions API Proxy ───────────────────────────────────────
// Keeps the provider API key server-side so it never reaches the browser.
// All three routes require a valid Supabase Bearer token.

const CLEANHEART_BASE_URL = 'https://cleanheartsolutions.com/api';
const CLEANHEART_API_KEY  = process.env.CLEANHEART_API_KEY || '';
...
app.get('/api/provider/balance', async (req, res) => {
  ...
  if (!user) return;
  if (!CLEANHEART_API_KEY) {
    return res.status(500).json({ success: false, error: 'Provider API key not configured.' });
  }
  try {
    const r = await fetch(`${CLEANHEART_BASE_URL}/balance`, {
      headers: { 'X-API-Key': CLEANHEART_API_KEY },
    });
    const data = await r.json();
    return res.status(r.status).json(data);
  } catch {
    return res.status(502).json({ success: false, error: 'Failed to reach provider.' });
  }
});

app.post('/api/provider/purchase', async (req, res) => {
  const user = await requireAuth(req, res);
  if (!user) return;
  if (!CLEANHEART_API_KEY) {
    return res.status(500).json({ success: false, error: 'Provider API key not configured.' });
  ...
  try {
    const r = await fetch(`${CLEANHEART_BASE_URL}/purchase`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-API-Key': CLEANHEART_API_KEY },
...
app.get('/api/provider/orders', async (req, res) => {
  ...
  if (!reference) {
    return res.status(400).json({ success: false, error: 'reference query param required.' });
  }
  try {
    const r = await fetch(
      `${CLEANHEART_BASE_URL}/orders?reference=${encodeURIComponent(reference)}`,
      { headers: { 'X-API-Key': CLEANHEART_API_KEY } }
    );
    const data = await r.json();
    return res.status(r.status).json(data);
